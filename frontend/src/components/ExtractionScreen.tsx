...
const templates = [
  ...
  { id: "luxuryhighend",     label: "Luxury High-End",    desc: "Dark gold editorial — cinematic & premium." },
...
function TemplatePreviewRenderer({
  ...
  switch (templateId) {
    case "glassmorphism":
      return <GlassmorphismPreview palette={defaultPalette} profile={profile} />;
    case "luxuryhighend":
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div style={{ width: "100%", maxWidth: "440px", flexShrink: 0 }}>
            <div style={{ backgroundColor: "rgba(189,184,185,0.05)", borderRadius: "16px", padding: "32px", border: "1px solid rgba(189,184,185,0.2)", height: "100%" }}>
              {/* Avatar + Name */}
              <div className="flex items-center gap-4 mb-6">
                <div style={{ width: "64px", height: "64px", borderRadius: "50%", backgroundColor: "#7F6269", display: "flex", alignItems: "center", justifyContent: "center", color: "#F4E1E0", fontSize: "20px", fontWeight: 700, flexShrink: 0 }}>
                  {initials}
                </div>
                <div>
                  <div style={{ color: "#F4E1E0", fontSize: "20px", fontWeight: 700 }}>{name}</div>
              ...
              {allSkills.length > 0 && (
                <div style={{ marginBottom: "20px" }}>
                  <div style={{ color: "#BDB8B9", fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "12px" }}>Skills</div>
                  <div className="flex flex-wrap gap-2">
                    {allSkills.map((s) => (
                      <span key={s} style={{ backgroundColor: "rgba(127,98,105,0.2)", color: "#F4E1E0", fontSize: "12px", fontWeight: 500, padding: "4px 12px", borderRadius: "999px" }}>{s}</span>
                    ))}
                  </div>
              ...
              {experienceList.length > 0 && (
                <div style={{ marginBottom: "20px" }}>
                  <div style={{ color: "#BDB8B9", fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "12px" }}>Experience</div>
                  {experienceList.map((e: any, idx: number) => (
                    <div key={idx} style={{ marginBottom: "12px" }}>
                      <div style={{ color: "#F4E1E0", fontSize: "14px", fontWeight: 600 }}>{e.role}</div>
              ...
              {education && (
                <div style={{ marginBottom: "20px" }}>
                  <div style={{ color: "#BDB8B9", fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "12px" }}>Education</div>
                  <div style={{ color: "#F4E1E0", fontSize: "14px", fontWeight: 600 }}>
                    {education.degree}{education.field ? ` in ${education.field}` : ""}
                  </div>
                  <div style={{ color: "#BDB8B9", fontSize: "13px" }}>
                    {education.institution}{education.startDate ? ` · ${education.startDate}` : ""}{education.endDate ? ` – ${education.endDate}` : ""}
                  </div>
              ...
              {achievementsList.length > 0 && (
                <div style={{ marginBottom: "24px" }}>
                  <div style={{ color: "#BDB8B9", fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "12px" }}>Achievements</div>
              ...
              <button
                ...
                style={{ backgroundColor: "transparent", color: "#F4E1E0", fontSize: "14px", fontWeight: 600, padding: "10px 24px", borderRadius: "999px", border: "1.5px solid #BDB8B9", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", width: "100%" }}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ backgroundColor: "rgba(189,184,185,0.05)", borderRadius: "16px", padding: "32px", border: "1px solid rgba(189,184,185,0.2)" }}>
              ...
              {/* 3-column grid to accommodate 6 templates */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px", marginBottom: "28px" }}>
                {templates.map((t) => (
                  <div
                    key={t.id}
                    onClick={() => setSelectedTemplate(t.id)}
                    style={{ backgroundColor: "rgba(189,184,185,0.04)", borderRadius: "12px", overflow: "hidden", border: selectedTemplate === t.id ? "2px solid #7F6269" : "1px solid rgba(189,184,185,0.2)", cursor: "pointer", position: "relative", transition: "border 0.15s ease" }}
                  >
                    {selectedTemplate === t.id && (
                      <div style={{ position: "absolute", top: "8px", right: "8px", width: "18px", height: "18px", backgroundColor: "#7F6269", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
                        <CheckCircle2 size={12} style={{ color: "#F4E1E0" }} />
                      </div>
                    )}
                    <div style={{ width: "100%", height: "100px", overflow: "hidden", pointerEvents: "none" }}>
                      <TemplatePreviewRenderer templateId={t.id} profile={profile} />
                    </div>
              ...
              <div className="flex gap-3 mb-8">
                {toneOptions.map((tone, i) => (
                  <button
                    ...
                    style={{ backgroundColor: selectedTone === i ? "#7F6269" : "transparent", color: "#F4E1E0", fontSize: "13px", fontWeight: selectedTone === i ? 600 : 400, padding: "8px 20px", borderRadius: "999px", border: selectedTone === i ? "none" : "1.5px solid rgba(189,184,185,0.5)", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s ease" }}
                  >
                    {tone}
                  </button>
                ))}
              ...
              <button
                ...
                style={{ backgroundColor: "#7F6269", color: "#F4E1E0", fontSize: "15px", fontWeight: 600, padding: "16px 0", borderRadius: "999px", border: "none", cursor: "pointer", width: "100%", fontFamily: "'DM Sans', sans-serif", boxShadow: "inset 0 1px 0 rgba(244,225,224,0.12), 0 4px 20px rgba(127,98,105,0.35)" }}
