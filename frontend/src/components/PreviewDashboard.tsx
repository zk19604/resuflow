...
const templateDefaultPalettes: Record<string, { name: string; colors: string[] }> = {
  bento:             { name: "Midnight Gold",  colors: ["#1A1A2E", "#F5C842", "#F2EFE9", "#6B7280",  "#4F46E5"] },
...
export function PreviewDashboard() {
  ...
  useEffect(() => {
    ...
    if (vibe) {
      try {
        ...
        if (ALL_TEMPLATE_IDS.includes(template as TemplateType)) {
  ...
  const templateOptions: { id: TemplateType; label: string }[] = ALL_TEMPLATE_IDS.map((id) => ({
    id,
