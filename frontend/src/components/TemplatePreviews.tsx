  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 2, textAlign: "center", padding: "0 24px", maxWidth: "90%" }}>
        ...
        <div style={{ fontSize: "clamp(20px, 4vw, 36px)", fontWeight: 800, color: "#FFFFFF", letterSpacing: "-0.02em", lineHeight: 1, marginBottom: "10px" }}>
          {name}
        </div>
        {summary && (
...
export function EditorialPreview({ profile }: { profile: any }) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: "#F2EDE4", display: "flex", flexDirection: "column", fontFamily: "'DM Sans', sans-serif", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 18px", borderBottom: "1px solid #D8D0C4" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        <div style={{ width: "52%", background: "#C8C0B4", position: "relative", display: "flex", alignItems: "flex-end", padding: "10px 12px" }}>
          <div style={{ background: "rgba(242,237,228,0.9)", border: "1px solid rgba(0,0,0,0.12)", padding: "3px 8px", fontSize: "6px", letterSpacing: "0.15em", color: "#333" }}>{role.toUpperCase()}</div>
        </div>
        <div style={{ width: "48%", padding: "16px 14px", display: "flex", flexDirection: "column", justifyContent: "center", gap: "6px", position: "relative" }}>
          ...
          <div style={{ lineHeight: 0.95 }}>
            <div style={{ fontSize: "clamp(18px, 3.5vw, 30px)", fontWeight: 900, color: "#1A1A1A", fontFamily: "Georgia, serif", letterSpacing: "-0.02em" }}>{firstName}</div>
            {lastName && (
              <div style={{ display: "flex", alignItems: "baseline" }}>
                <span style={{ fontSize: "clamp(16px, 3vw, 26px)", fontWeight: 900, fontStyle: "italic", color: "#1A1A1A", fontFamily: "Georgia, serif", letterSpacing: "-0.02em" }}>{lastName.slice(0, -2)}–</span>
              </div>
            )}
            {lastName && <div style={{ fontSize: "clamp(16px, 3vw, 26px)", fontWeight: 900, color: "#C0392B", fontFamily: "Georgia, serif", letterSpacing: "-0.02em" }}>{lastName.slice(-2).toUpperCase()}</div>}
          </div>
          ...
          <div style={{ marginTop: "6px", background: "#1A1A1A", color: "#F2EDE4", fontSize: "6px", fontWeight: 700, letterSpacing: "0.12em", padding: "5px 10px", display: "inline-block", width: "fit-content" }}>VIEW MY WORK →</div>
          <div style={{ position: "absolute", bottom: "8px", right: "10px", fontSize: "36px", fontWeight: 900, color: "rgba(0,0,0,0.06)", fontFamily: "Georgia, serif", lineHeight: 1, pointerEvents: "none" }}>01</div>
        </div>
      </div>
    </div>
  );
...
export function BentoPreview({ profile }: { profile: any }) {
  ...
  const topSkills: string[] = [...(profile?.skills?.technical || []), ...(profile?.skills?.tools || [])].slice(0, 5);
  const projectCount = profile?.projects?.length || 0;
  const expCount = profile?.workExperience?.length || 0;

  return (
    <div style={{ width: "100%", height: "100%", background: "#F8F8F8", display: "grid", gridTemplateColumns: "2fr 1.2fr 1.2fr 1fr", gridTemplateRows: "1fr 1fr", gap: "6px", padding: "8px", fontFamily: "'DM Sans', sans-serif", overflow: "hidden", boxSizing: "border-box" }}>
      <div style={{ background: "#1A1A2E", borderRadius: "10px", padding: "12px", display: "flex", flexDirection: "column", justifyContent: "space-between", overflow: "hidden", gridRow: "1 / 2" }}>
        <div>
          <div style={{ fontSize: "6px", letterSpacing: "0.15em", color: "rgba(255,255,255,0.4)", textTransform: "uppercase", marginBottom: "4px" }}>{role}</div>
          <div style={{ fontSize: "clamp(14px, 2.5vw, 22px)", fontWeight: 900, color: "#FFFFFF", lineHeight: 1, letterSpacing: "-0.02em" }}>{name.split(" ")[0].toUpperCase()}</div>
          <div style={{ fontSize: "clamp(14px, 2.5vw, 22px)", fontWeight: 900, color: "#F5C518", lineHeight: 1, letterSpacing: "-0.02em" }}>{name.split(" ").slice(1).join(" ").toUpperCase()}</div>
        </div>
        {summary && <div style={{ fontSize: "6px", color: "rgba(255,255,255,0.45)", lineHeight: 1.5, marginTop: "6px" }}>{summary.slice(0, 80)}{summary.length > 80 ? "…" : ""}</div>}
      ...
      <div style={{ background: "#FFFFFF", borderRadius: "10px", padding: "10px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "5px", overflow: "hidden" }}>
      ...
      <div style={{ background: "#F5C518", borderRadius: "10px", padding: "10px", display: "flex", flexDirection: "column", justifyContent: "space-around", overflow: "hidden" }}>
        <div style={{ fontSize: "5px", letterSpacing: "0.15em", color: "rgba(0,0,0,0.5)", textTransform: "uppercase" }}>By The Numbers</div>
        {[{ val: `${projectCount || 3}+`, label: "Projects Shipped" }, { val: `${expCount || 2}+`, label: "Years Experience" }].map((s) => (
          <div key={s.label}>
      ...
      <div style={{ background: "#6C5CE7", borderRadius: "10px", padding: "10px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "flex-start", gap: "8px", overflow: "hidden" }}>
      ...
      <div style={{ background: "#FFFFFF", borderRadius: "10px", padding: "10px", overflow: "hidden", gridColumn: "1 / 2" }}>
        ...
        {summary && <div style={{ fontSize: "5px", color: "#666", lineHeight: 1.5 }}>{summary.slice(0, 100)}{summary.length > 100 ? "…" : ""}</div>}
      </div>
      <div style={{ background: "#1A1A2E", borderRadius: "10px", padding: "10px", overflow: "hidden", gridColumn: "2 / 5" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
          {topSkills.map((skill) => (
            <div key={skill} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "999px", padding: "2px 8px", fontSize: "5px", color: "rgba(255,255,255,0.7)" }}>{skill}</div>
          ))}
          {topSkills.length === 0 && <div style={{ fontSize: "6px", color: "rgba(255,255,255,0.3)" }}>Skills will appear here</div>}
        </div>
      </div>
    </div>
  );
